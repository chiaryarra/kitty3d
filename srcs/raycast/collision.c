...
/*   Updated: 2025/07/17 18:47:36 by lbarreto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int can_move_front_back(t_data *d, double x, double y, double move_vector)
{
    ...
    if (mx < 0)
        return (0);
    if (mx >= d->map->grid_x_size)    /* largura fixa do stub */
        return (0);
    if (my < 0)
        return (0);
    if (my >= d->map->grid_y_size)    /* altura fixa do stub */
		return (0);
    /* parede: */
	// printf("check x: %d check y: %d pov x: %f pov y: %f mv: %f\n", (int)check_x, (int)check_y, d->raycast->pov_x, d->raycast->pov_y, move_vector);
